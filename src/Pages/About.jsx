...
const founders = [
  {
    ...
    avatar: 'SB',
    photo: null, // Set to imported image: import sabyPhoto from '../assets/saby.jpg'
...
Then came Northeast India. A slow road through Meghalaya. Root bridges older than anything he'd ever seen. Villages with no WiFi and infinite stories. Stays that felt like being welcomed into someone's life rather than their inventory. He looked for a platform that could do those places justice. There wasn't one. That absence became Hoppity.`,
  ...
  },
  {
    ...
    avatar: 'DJ',
    photo: null, // Set to imported image: import dipakPhoto from '../assets/dipak.jpg'
...
When Saby came to him with Hoppity, Dipak saw something most people miss: this isn't just a travel app. It's a data platform, a discovery engine, a community. He came back to build the infrastructure that could carry that vision — a backend that can scale from a few hundred travellers to millions without losing its soul.`,
  ...
  },
  {
    ...
    linkedin: null,
    avatar: 'AS',
    photo: null, // Set to imported image: import ajayPhoto from '../assets/ajay.jpg'
    ...
    <div className="min-h-screen bg-[#f7f1ff]">
      ...
      <section className="relative overflow-hidden pt-28 pb-16">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 lg:px-10 text-center text-white">
          ...
          <div className="mb-10">
            ...
            <blockquote className="text-3xl md:text-4xl lg:text-5xl font-black leading-[1.1] tracking-[-0.02em] text-white mb-6">
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-6 lg:px-10 py-16">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-600 mb-4">The gap they couldn't unsee</p>
              <h2 className="text-3xl md:text-4xl font-black tracking-tight text-slate-950 leading-tight mb-6">
      ...
      <div className="bg-slate-50 border-y border-slate-100 py-5">
        <div className="mx-auto max-w-5xl px-6">
          <div className="flex items-center gap-4 flex-wrap justify-center">
            ...
            {brandLogos.map(b => (
              <span key={b.name} className="text-sm font-black tracking-tight px-3 py-1.5 rounded-lg"
      ...
      <section className="mx-auto max-w-6xl px-6 lg:px-10 py-16 space-y-24">
        ...
        {founders.map((founder, idx) => (
          <div key={founder.name}
            className={`grid lg:grid-cols-[1fr_1.6fr] gap-12 lg:gap-16 items-start ${idx % 2 === 1 ? 'lg:grid-cols-[1.6fr_1fr]' : ''}`}>

            {/* Left: Identity card */}
            <div className={idx % 2 === 1 ? 'lg:order-2' : ''}>
              {/* Avatar */}
              {founder.photo ? (
                <div className="w-20 h-20 rounded-3xl overflow-hidden mb-6 shadow-lg ring-4 ring-white">
                  <img src={founder.photo} alt={founder.name} className="w-full h-full object-cover object-top" />
                </div>
              ) : (
                <div className={`w-20 h-20 rounded-3xl bg-gradient-to-br ${founder.gradient} flex items-center justify-center text-white text-2xl font-black mb-6 shadow-lg`}>
                  {founder.avatar}
                </div>
              )}

              <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-600 mb-2">{founder.role}</p>
              <h3 className="text-2xl md:text-3xl font-black tracking-tight text-slate-950 mb-1">{founder.name}</h3>
              <p className="text-sm text-slate-500 flex items-center gap-1.5 mb-6">
                <MapPin className="w-3.5 h-3.5" />{founder.origin}
              </p>
            ...
            <div className={idx % 2 === 1 ? 'lg:order-1' : ''}>
              <div className="rounded-3xl bg-white border border-violet-100 p-8 lg:p-10 shadow-sm">
                ...
                <div className="space-y-5">
                  {founder.story.split('\n\n').map((para, i) => (
                    <p key={i} className={`leading-8 text-slate-700 ${i === 0 ? 'text-lg font-medium text-slate-800' : 'text-base'}`}>
                      {para}
                    </p>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-8 pt-6 border-t border-slate-100">
                  {founder.companies.map(c => (
                    <span key={c} className="text-xs font-bold text-slate-500 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-full">
                      {c}
                    </span>
                  ))}
                </div>
      ...
      <section className="bg-slate-950 py-16">
        <div className="mx-auto max-w-5xl px-6 lg:px-10">
          <div className="text-center mb-12">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-400 mb-4">The whole is greater</p>
            <h2 className="text-3xl md:text-4xl font-black text-white tracking-tight leading-tight max-w-3xl mx-auto">
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-5xl px-6 lg:px-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-600 mb-4">Why we built this</p>
              <h2 className="text-3xl md:text-4xl font-black tracking-tight text-slate-950 leading-tight mb-6">
                Travel should return you to yourself. Most of it doesn't.
              </h2>
              <div className="space-y-4 text-slate-600 leading-8">
                <p>Most platforms are built for transactions. They have no opinion about where you go, as long as you book. They have no stake in whether the trip changes you.</p>
                <p>We have an opinion. We believe the hidden village matters more than the five-star hotel. We believe the host who cooks from memory matters more than the one who hired a chef. We believe a journey should cost you something — not just money, but attention, presence, willingness to be surprised.</p>
                <p className="font-semibold text-slate-800">That's not a product feature. That's a worldview. And it's why this team exists.</p>
              </div>
            </div>

            {/* Mission statement card */}
            <div className="rounded-3xl bg-gradient-to-br from-violet-700 via-violet-800 to-fuchsia-800 p-10 text-white shadow-2xl">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-violet-200 mb-5">The Hoppity mission</p>
              ...
              <div className="space-y-3 text-sm text-violet-100">
                {[
                  ...
                  'Story-led, not stock-photo-led',
                  'Built on trust between travellers and hosts',
                ].map(b => (
      ...
      <section className="bg-[#f7f1ff] py-14 border-y border-violet-100">
        <div className="mx-auto max-w-5xl px-6 lg:px-10 text-center">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-violet-600 mb-3">Already on the ground</p>
          <h3 className="text-2xl font-black text-slate-950 mb-8">
            Ajay's first call resulted in 7 partner networks. That's what 30 years looks like.
          </h3>
          <div className="flex flex-wrap justify-center gap-3">
            {['Paramount', 'AV Tours', 'Villotale', 'Chalohoppo', 'Magic Moments', 'Clubside', 'Help Tourism'].map(p => (
              <span key={p} className="bg-white border border-violet-200 text-slate-700 text-sm font-semibold px-4 py-2 rounded-full shadow-sm">
                {p}
              </span>
            ))}
          </div>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-4xl px-6 lg:px-10 text-center">
          <h2 className="text-3xl md:text-4xl font-black tracking-tight text-slate-950 mb-5">
            The trip that started this company is still out there.
          </h2>
          <p className="text-lg text-slate-600 max-w-xl mx-auto mb-10 leading-8">
      ...
      <footer className="border-t border-violet-100 bg-white py-6">
        <div className="mx-auto max-w-6xl px-6 lg:px-10 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-500">
