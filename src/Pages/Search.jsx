...
async function aiSearch(query, history = []) {
  ...
  const headers = {
    ...
    'Authorization': session ? `Bearer ${session.access_token}` : `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Indlbmh1ZGN5dmxoaWxwZ2F6eWxnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk0OTY0MTgsImV4cCI6MjA4NTA3MjQxOH0.Jdx993pFvb0JC87NaYhOQ6UR_7UIJBA1mkFQUeoK7bA'}`,
  }
  const res = await fetch('https://wenhudcyvlhilpgazylg.supabase.co/functions/v1/ai-search', {
    method: 'POST',
    headers,
...
const TRENDING = ['Meghalaya', 'Spiti Valley', 'Ladakh', 'Rajasthan', 'Kerala', 'Wildlife', 'Heritage', 'Himalayas']
const PROMPT_CHIPS = ['✨ Surprise me', '🌿 Offbeat Northeast', '👨‍👩‍👧 Family friendly', '💰 Under ₹2,000', '🏔️ Himalayan trek', '🕉️ Spiritual journey', '🐘 Wildlife safari', '⚡ Weekend trip']

export default function SearchPage() {
  const [mode, setMode] = useState('fts')        // 'fts' | 'ai'
  const [query, setQuery] = useState('')
  const [results, setResults] = useState([])
  const [aiReply, setAiReply] = useState(null)   // { explanation, follow_up_suggestions }
  ...
  const saveRecent = (q) => {
    ...
    localStorage.setItem('hoppity_recent', JSON.stringify(updated))
  }

  // ── FTS search via Supabase RPC ────────────────────────────────────
  ...
  const handleSearch = async (q = query) => {
    ...
    if (mode === 'ai') {
      try {
        ...
        setResults([]); setAiReply({ explanation: 'Sorry, AI search is unavailable. Please try the standard search.', followUps: [] })
      }
    } else {
  ...
  const handleChip = (chip) => {
    const q = chip.replace(/^[\S]{1,2}\s/, '')
    setQuery(q)
    handleSearch(q)
  }

  const clearSearch = () => {
    setQuery(''); setResults([]); setSearched(false); setAiReply(null)
    setAiHistory([])
    inputRef.current?.focus()
  }
      ...
      <div className="sticky top-20 z-30 bg-[#f7f1ff]/95 backdrop-blur border-b border-violet-100 px-4 py-4 mt-20">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder={mode === 'ai' ? 'Describe your dream trip…' : 'Search tours, places, experiences…'}
          ...
          <div className="flex gap-2">
            <button
              onClick={() => { setMode('fts'); clearSearch() }}
            ...
            <button
              onClick={() => { setMode('ai'); clearSearch() }}
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-semibold transition cursor-pointer ${
                mode === 'ai'
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        {!searched && (
          <>
            ...
            {mode === 'ai' && (
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Try asking…</p>
                <div className="flex flex-wrap gap-2">
                  {PROMPT_CHIPS.map(chip => (
                    <button key={chip} onClick={() => handleChip(chip)}
                      className="px-4 py-2 rounded-full bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 text-sm font-semibold text-violet-700 hover:from-violet-100 hover:to-purple-100 transition cursor-pointer">
                      {chip}
                    </button>
                  ))}
                </div>
        ...
        {/* AI explanation */}
        {aiReply && (
          <div className="mb-6 rounded-2xl bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-200 p-5">
          ...
          <>
            {results.length > 0 ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-slate-500 text-sm mt-2">Try different keywords or{' '}
                  <button onClick={() => setMode(mode === 'ai' ? 'fts' : 'ai')} className="text-violet-600 hover:underline cursor-pointer">
                    switch to {mode === 'ai' ? 'standard' : 'AI'} search
                  </button>
                </p>
              </div>
            )}
  ...
  return (
    <Link to={`/itinerary/${tour.slug}`}
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-slate-500 mt-1">📍 {tour.location}</p>
