...
function ToolbarBtn({ onClick, title, active, children }) {
  return (
    <button
      ...
      title={title}
      className={`w-8 h-8 flex items-center justify-center rounded-lg transition cursor-pointer text-sm ${
      ...
      {value.length < 8 && (
        <div className="flex items-center gap-1">
          <input
            ...
            placeholder="Add tag…"
            className="text-xs border border-slate-200 rounded-full px-3 py-1.5 outline-none focus:border-violet-400 w-28"
          />
...
export default function BlogComposePage() {
  ...
  const uploadCover = async (file) => {
    ...
    if (file.size > MAX_COVER_MB * 1024 * 1024) {
      showToast('error', `Cover must be under ${MAX_COVER_MB}MB`)
      return
    }
  ...
  const submitForReview = async () => {
    ...
    if (!window.confirm('Submit this story for review? The Hoppity team will review and approve it.')) return
    setSubmitting(true)
    ...
    showToast('success', 'Submitted for review! We\'ll notify you when it\'s live.')
  ...
  return (
    <div className="min-h-screen bg-[#f7f1ff]">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-24 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-sm font-semibold border ${
          toast.type === 'success'
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <Link to="/profile?tab=stories"
              className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-violet-700 transition">
              <ArrowLeft className="w-4 h-4" /> My Stories
            </Link>
            {postStatus !== 'draft' && (
              <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                ...
                {postStatus === 'pending' ? '⏳ Under review' :
                 postStatus === 'approved' ? '✓ Published' : postStatus}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={saveDraft}
              disabled={saving}
              className="flex items-center gap-1.5 text-sm font-semibold text-slate-600 hover:text-violet-700 bg-white border border-slate-200 rounded-xl px-4 py-2 transition cursor-pointer disabled:opacity-50">
              {saving ? <Loader className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              {saving ? 'Saving…' : 'Save draft'}
            </button>
            {(postStatus === 'draft' || postStatus === 'on_hold') && (
              <button
                onClick={submitForReview}
                disabled={submitting}
                className="flex items-center gap-1.5 text-sm font-semibold text-white bg-violet-700 hover:bg-violet-800 rounded-xl px-4 py-2 transition cursor-pointer disabled:opacity-50 shadow-sm">
        ...
        {preview ? (
          ...
          <div className="bg-white rounded-3xl border border-violet-100 overflow-hidden shadow-sm">
            ...
            <div className="p-8 md:p-12">
              ...
              <h1 className="text-3xl md:text-4xl font-black text-slate-950 mt-3 mb-6 leading-tight">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-violet-100 overflow-hidden">
              {coverUrl ? (
                <div className="relative h-52 group">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3">
                    <button
                      ...
                      className="bg-white text-slate-900 font-semibold text-sm px-4 py-2 rounded-xl cursor-pointer hover:bg-slate-50 transition">
                      Change cover
                    </button>
                    <button
                      type="button"
                      onClick={() => setCoverUrl('')}
                      className="bg-white/20 text-white font-semibold text-sm px-4 py-2 rounded-xl cursor-pointer hover:bg-white/30 transition">
                      Remove
                    </button>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full h-40 flex flex-col items-center justify-center gap-3 text-slate-400 hover:text-violet-600 hover:bg-violet-50 transition cursor-pointer group">
                  {coverUploading ? (
                    <><Loader className="w-6 h-6 animate-spin text-violet-600" /><span className="text-sm font-medium text-violet-600">Uploading…</span></>
                  ) : (
                    <><Upload className="w-6 h-6 group-hover:scale-110 transition" /><span className="text-sm font-medium">Add cover image</span><span className="text-xs">JPG, PNG up to 3MB</span></>
                  )}
                </button>
              )}
            ...
            <input
              ...
              maxLength={120}
              className="w-full text-3xl md:text-4xl font-black text-slate-950 placeholder-slate-300 bg-transparent border-none outline-none leading-tight"
            />
            ...
            <div className="flex flex-wrap items-center gap-4 py-4 border-y border-slate-100">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex-shrink-0">Tags</span>
            ...
            <div className="bg-white rounded-3xl border border-violet-100 overflow-hidden shadow-sm">
              ...
              <div className="flex items-center gap-0.5 px-4 py-2.5 border-b border-slate-100 flex-wrap">
                ...
                <ToolbarBtn onClick={() => exec('bold')} title="Bold" active={queryCmd('bold')}>
                  <Bold className="w-3.5 h-3.5" />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('italic')} title="Italic" active={queryCmd('italic')}>
                  <Italic className="w-3.5 h-3.5" />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => exec('underline')} title="Underline" active={queryCmd('underline')}>
                ...
                <div className="w-px h-5 bg-slate-200 mx-1" />
                <ToolbarBtn onClick={() => setShowLinkInput(l => !l)} title="Insert link">
                  <Link2 className="w-3.5 h-3.5" />
                </ToolbarBtn>
                <ToolbarBtn onClick={() => inlineImgInputRef.current?.click()} title="Insert image">
                ...
                {/* Link input */}
                {showLinkInput && (
                  <div className="flex items-center gap-2 ml-2 border border-violet-200 rounded-xl px-3 py-1.5 bg-violet-50">
                    <input
                      value={linkUrl}
                      onChange={e => setLinkUrl(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && insertLink()}
                      placeholder="Paste URL and press Enter"
                      className="text-xs outline-none bg-transparent w-48 text-slate-700"
                      autoFocus
                    />
                    <button type="button" onClick={insertLink}
                      className="text-xs font-bold text-violet-700 cursor-pointer">Add</button>
                    <button type="button" onClick={() => { setShowLinkInput(false); setLinkUrl('') }}
                      className="cursor-pointer"><X className="w-3 h-3 text-slate-400" /></button>
                  </div>
                )}
              ...
              <div
                ...
                className="blog-prose min-h-[420px] px-8 py-7 outline-none"
                style={{ caretColor: '#7c3aed' }}
                onInput={() => {}} // triggers re-render for toolbar state
              />
            </div>

            {/* Writing tips */}
            <div className="bg-violet-50 rounded-2xl p-5 border border-violet-100">
              <p className="text-xs font-bold text-violet-700 uppercase tracking-wider mb-2">💡 Tips for a great story</p>
              <ul className="text-sm text-slate-600 space-y-1">
                <li>• Be specific — name the place, describe the smell, the light, the moment</li>
                <li>• Use photos to break up long text (upload via the image button in the toolbar)</li>
                <li>• Stories under review are published within 24–48 hours</li>
              </ul>
            </div>
          </div>
        )}
      ...
      {/* Blog prose styles (shared with BlogPost.jsx) */}
      <style>{`
        .blog-prose { font-size: 17px; line-height: 1.85; color: #1e293b; }
